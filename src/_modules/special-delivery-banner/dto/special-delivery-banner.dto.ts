...
// are provided). Mirrors BannerDTO's IsAfterStartDate.
@ValidatorConstraint({ name: 'isAfterSpecialDeliveryBannerStartDate', async: false })
...
export class CreateSpecialDeliveryBannerDTO {
  @Required()
  @ValidateName({
    example: '{"ar":"بانر توصيل خاص","en":"Special Delivery Banner"}',
  })
  name: Json;
  ...
  // Zone / area targeting. Allowed zones are those reachable through the
  // selected store's branches (Store -> Branch -> BranchZone -> Zone).
  @Optional()
...
export class UpdateSpecialDeliveryBannerDTO extends PartialType(
...
export class SortSpecialDeliveryBannerDTO {
  @SortProp()
...
export class FilterSpecialDeliveryBannerDTO extends PaginationParamsDTO {
  @Optional()
  @ValidateNumber()
  id?: Id;
  ...
  @Optional()
  orderBy?: SortSpecialDeliveryBannerDTO[];
}
