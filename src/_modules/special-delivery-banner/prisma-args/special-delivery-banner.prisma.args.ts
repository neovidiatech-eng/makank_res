import { Language, Prisma, SpecialDeliveryBanner } from '@prisma/client';
...
import { FilterSpecialDeliveryBannerDTO } from '../dto/special-delivery-banner.dto';
...
const scheduleWindow = (
  now: Date,
): Prisma.SpecialDeliveryBannerWhereInput => ({
  AND: [
...
export const getSpecialDeliveryBannerArgs = (
  query: FilterSpecialDeliveryBannerDTO,
  languages: Language[],
  isCustomer = false,
) => {
  const { page, limit, orderBy, ...filter } = query;
  const searchArray = [
    filterKey<SpecialDeliveryBanner>(filter, 'id'),
    filterJsonKeyWithRawSQL(filter, 'name', languages),
    filterKey<SpecialDeliveryBanner>(filter, 'storeId'),
    filterKey<SpecialDeliveryBanner>(filter, 'categoryId'),
    filterKey<SpecialDeliveryBanner>(filter, 'serviceId'),
    filterKey<SpecialDeliveryBanner>(filter, 'targetType'),
    filterKey<SpecialDeliveryBanner>(filter, 'active'),
  ].filter(Boolean) as Prisma.SpecialDeliveryBannerWhereInput[];
  ...
  ) as Prisma.SpecialDeliveryBannerOrderByWithRelationInput[];
  ...
  } as Prisma.SpecialDeliveryBannerFindManyArgs;
};

export const selectSpecialDeliveryBannerOBJ = (userId?: Id) => {
  const selectArgs: Prisma.SpecialDeliveryBannerSelect = {
    id: true,
    name: true,
    image: true,
    active: true,
    ...
    // boolean in SpecialDeliveryBannerService.mapBanner and the raw rows are
    // never returned.
    Store: {
      select: {
        id: true,
...
export const getSpecialDeliveryBannerArgsWithSelect = (userId?: Id) => {
  return {
    select: selectSpecialDeliveryBannerOBJ(userId),
  } satisfies Prisma.SpecialDeliveryBannerFindManyArgs;
};
